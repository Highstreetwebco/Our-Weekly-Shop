...
export function HomeDashboard({ shop, basket, stage, goStep, openPerson, setModal, setTab, startNextWeek, Button }) {
  ...
  return <>
    ...
    <View style={d.rowBetween}><Text accessibilityRole="header" style={d.sectionTitle}>Your week, at a glance</Text><Button label="Open my plan" secondary small onPress={() => goStep(0)} /></View>
    ...
    <View style={d.repeat}><View style={[d.flex,{gap:7}]}><Text style={d.tag}>NEXT WEEK, MADE EASIER</Text><Text accessibilityRole="header" style={d.sectionTitle}>A good week is worth repeating.</Text><Text style={d.body}>Copy these meals and extras into next week. Change anything that doesn’t fit.</Text></View><Button label="Use this plan next week" secondary disabled={!started} onPress={startNextWeek} />{!started && <Text style={d.small}>Add a meal or an extra item to try this.</Text>}</View>
    <View style={d.rowBetween}><View><Text style={d.eyebrow}>KEEP THE GOOD ONES CLOSE</Text><Text accessibilityRole="header" style={d.sectionTitle}>Meals you can come back to</Text></View><Button label="See all meals" secondary small onPress={() => setTab('Recipes')} /></View>
...
export function WeekBoard({ shop, onOpenDay, onMeal, onReuse }) {
  const wide = useWindowDimensions().width >= 760;
  const w = currentWeek(shop);
  return <View style={d.board}>{DAYS.map(day => <View key={day} style={[d.dayCard,wide && {width:'48.8%'}]}><View style={d.rowBetween}><Text accessibilityRole="header" style={d.dayTitle}>{day}</Text><Action label={`Edit ${day}`} onPress={() => onOpenDay(day)} style={d.dayEdit}><Text style={d.editText}>Edit day</Text><Ionicons name="chevron-forward" size={17} color={C.primary} /></Action></View>{SLOTS.map(slot => {
