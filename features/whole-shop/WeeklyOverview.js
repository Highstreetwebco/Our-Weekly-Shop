...
export function HomeDashboard({ shop, basket, stage, goStep, openPerson, setModal, setTab, startNextWeek, Button }) {
  ...
  return <>
    ...
    <View style={d.boundary}><Ionicons name="information-circle-outline" size={23} color={C.primary} /><View style={d.flex}><Text style={d.cardTitle}>Basket planning is ready to use</Text><Text style={d.small}>Sainsbury’s catalogue search is available in Basket. An enabled private pilot can add products only after you approve them; checkout, payment and placing the order always stay on Sainsbury’s.</Text></View></View>
  </>;
}
