...
function SetupScreen({ household, onComplete }) {
  ...
  const quickIngredients = INGREDIENT_CATALOG;
  ...
  const suggestions = ingredient.trim() ? quickIngredients.filter((item) => item.toLowerCase().includes(ingredient.toLowerCase())).slice(0, 8) : [];
