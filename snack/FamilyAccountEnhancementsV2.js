...
export default function FamilyAccountEnhancementsV2(){
  ...
  useEffect(()=>{
    ...
    const locate=()=>{
      ...
      const accountTitle=texts.some(el=>el.children.length===0&&String(el.textContent||"").trim()==="Account details.");
      const accountNav=texts.some(el=>el.children.length===0&&String(el.textContent||"").trim()==="Account");
      setAccountVisible(accountTitle&&accountNav);
    };
    locate();
    const obs=new MutationObserver(locate);obs.observe(document.body,{subtree:true,childList:true,characterData:true});
  ...
  async function saveMember(){
    ...
    const next=[...members,member];setMembers(next);writeFamily(next);setStatus("Saving…");
    try{
      const {data}=await supabase.auth.getSession();const uid=data?.session?.user?.id;
      if(uid){
        const row=await supabase.from("profiles").select("app_state").eq("id",uid).maybeSingle();const state=row.data?.app_state||{};
        ...
        if(hm.data?.household_id)await supabase.from("household_people").insert({household_id:hm.data.household_id,name:clean,role,portion_multiplier:member.portionMultiplier});
      }
      setStatus(`${clean} added to your planning group.`);
    }catch{setStatus(`${clean} added on this device.`)}
  ...
  return <>
    <AppShellFixes/>
    {accountVisible&&!show&&!celebrate?<View pointerEvents="box-none" style={StyleSheet.absoluteFill}><TouchableOpacity style={s.familyCta} onPress={()=>setShow(true)}><View style={{flex:1}}><Text style={s.familyTitle}>Family & planning group</Text><Text style={s.familySub}>{members.length?`${members.length} saved · add or edit members`:"Add adults, children and babies"}</Text></View><Ionicons name="add-circle" size={27} color={C.green}/></TouchableOpacity></View>:null}

    {show&&<View style={s.overlay}><SafeAreaView style={s.safe}><View style={s.head}><View><Text style={s.kicker}>MY ACCOUNT</Text><Text style={s.title}>Family & group.</Text></View><TouchableOpacity onPress={()=>setShow(false)}><Ionicons name="close" size={28} color={C.muted}/></TouchableOpacity></View><ScrollView contentContainerStyle={s.page} keyboardShouldPersistTaps="handled">
      <Text style={s.lead}>These are the people Gemma can plan meals and portions for. Add adults, children or babies here and they will be available when you choose who is eating.</Text>
      {members.length?<View style={{marginBottom:10}}>{members.map(m=><View key={m.id} style={s.member}><View style={{flex:1}}><Text style={s.memberName}>{m.name}</Text><Text style={s.memberMeta}>{ageText(m)}</Text></View><TouchableOpacity onPress={()=>removeMember(m.id)}><Ionicons name="trash-outline" size={20} color={C.red}/></TouchableOpacity></View>)}</View>:<View style={s.emptyMembers}><Ionicons name="people-outline" size={24} color={C.green}/><Text style={s.emptyText}>No family members added here yet.</Text></View>}
      <View style={s.card}><Text style={s.section}>ADD A NEW MEMBER</Text><Text style={s.label}>NAME</Text><TextInput style={s.input} value={name} onChangeText={setName} placeholder="e.g. Sophie" placeholderTextColor={C.muted}/><Text style={s.label}>TYPE</Text><View style={s.row}>{["Adult","Child"].map(x=><TouchableOpacity key={x} style={[s.chip,role===x&&s.chipOn]} onPress={()=>setRole(x)}><Text style={[s.chipText,role===x&&s.chipTextOn]}>{x}</Text></TouchableOpacity>)}</View>
