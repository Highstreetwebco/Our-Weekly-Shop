...
export default function FamilyAccountEnhancementsV2(){
  ...
  useEffect(()=>{
    ...
    const locate=()=>{
      const texts=[...document.querySelectorAll("span,div,p")];
      const title=texts.find(el=>el.children.length===0&&String(el.textContent||"").trim()==="Account details.");
      const saveText=texts.find(el=>el.children.length===0&&String(el.textContent||"").trim()==="Save account details");
      if(!title||!saveText){setAccountButton(null);return}
      let save=saveText;while(save&&save!==document.body&&save.getAttribute?.("role")!=="button"&&save.tagName!=="BUTTON")save=save.parentElement;
      if(!save||save===document.body){setAccountButton(null);return}
      let card=save.parentElement;
      while(card&&card!==document.body){const r=card.getBoundingClientRect();if(r.width>280&&r.height>300&&r.height<900)break;card=card.parentElement}
      if(!card||card===document.body){setAccountButton(null);return}
      const r=card.getBoundingClientRect();
      const nav=[...document.querySelectorAll("span,div")].find(el=>el.children.length===0&&String(el.textContent||"").trim()==="Account")?.getBoundingClientRect();
      const maxBottom=(nav?.top||window.innerHeight-82)-10;
      const top=Math.min(r.bottom+12,maxBottom-68);
      if(top<r.top||top<100){setAccountButton(null);return}
      setAccountButton({top,left:r.left,width:r.width});
    };
    locate();
    const obs=new MutationObserver(locate);obs.observe(document.body,{subtree:true,childList:true,characterData:true,attributes:true,attributeFilter:["style"]});
    const timer=setInterval(locate,300);
    window.addEventListener("resize",locate);
    return()=>{obs.disconnect();clearInterval(timer);window.removeEventListener("resize",locate)};
  },[]);
